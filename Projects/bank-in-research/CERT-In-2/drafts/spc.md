# Responsible Disclosure: Dangling DNS Records on `SPC.bank.in` (1 records)
...
| Entity | SPC (spc.bank.in registrant) (spc) |
...
www.spc.bank.in CNAMEs to 162.241.85.151.

**Affected hostnames (1 records):** `www.spc.bank.in`.
...
- Universe: the full published `spc.bank.in` zone from the IDRBT-derived
...
| Dangling target | `162.241.85.151` |

The bank's authoritative nameservers still publish this record, but the
service it points to is gone:

```text
$ dig +noall +answer www.spc.bank.in CNAME
www.spc.bank.in.  14400  IN  CNAME  162.241.85.151.

$ dig www.spc.bank.in   # -> status: NXDOMAIN
