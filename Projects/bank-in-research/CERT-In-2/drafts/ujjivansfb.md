# Responsible Disclosure: Dangling DNS Records on `UJJIVANSFB.bank.in` (1 records)
...
| Entity | Ujjivan Small Finance Bank Ltd (ujjivansfb) |
...
nearmeuat.ujjivansfb.bank.in -> ujjivan-uat.singleinterface.com. SingleInterface is a store-locator/marketing SaaS; the UAT pointer may outlive the engagement.

**Affected hostnames (1 records):** `nearmeuat.ujjivansfb.bank.in`.
...
- Universe: the full published `ujjivansfb.bank.in` zone from the IDRBT-derived
...
### F1 - `nearmeuat.ujjivansfb.bank.in`

| Field | Value |
| --- | --- |
| Severity | **MEDIUM** |
| Issue class | Dangling CNAME (miscellaneous target) |
| Dangling target | `ujjivan-uat.singleinterface.com` |

The bank's authoritative nameservers still publish this record, but the
service it points to is gone:

```text
$ dig +noall +answer nearmeuat.ujjivansfb.bank.in CNAME
nearmeuat.ujjivansfb.bank.in. 60 IN  CNAME  ujjivan-uat.singleinterface.com.

$ dig nearmeuat.ujjivansfb.bank.in   # -> status: NOERROR
