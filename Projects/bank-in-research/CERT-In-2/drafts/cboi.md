# Responsible Disclosure: Dangling DNS Records on `CBOI.bank.in` (1 records)
...
| Entity | Central Bank of India (cboi.zone) (cboi) |
...
www.cboi.bank.in CNAMEs to the apex cboi.bank.in, which itself has no A record in the release snapshot - a self-referential loop that browsers cannot resolve.

**Affected hostnames (1 records):** `www.cboi.bank.in`.
...
- Universe: the full published `cboi.bank.in` zone from the IDRBT-derived
...
| Dangling target | `cboi.bank.in` |

The bank's authoritative nameservers still publish this record, but the
service it points to is gone:

```text
$ dig +noall +answer www.cboi.bank.in CNAME
www.cboi.bank.in.  14400  IN  CNAME  cboi.bank.in.

$ dig www.cboi.bank.in   # -> status: NOERROR
...
Availability of the www name; reputational.
