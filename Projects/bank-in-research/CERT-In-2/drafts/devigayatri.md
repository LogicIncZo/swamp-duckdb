# Responsible Disclosure: Dangling DNS Records on `DEVIGAYATRI.bank.in` (1 records)
...
| Entity | Devi Gayatri (co-operative bank registrant) (devigayatri) |
...
www.devigayatri.bank.in CNAMEs to 43.255.154.56 (shared hosting IP also referenced by jagruticoop - same DNS vendor template).

**Affected hostnames (1 records):** `www.devigayatri.bank.in`.
...
- Universe: the full published `devigayatri.bank.in` zone from the IDRBT-derived
...
### F1 - `www.devigayatri.bank.in`

| Field | Value |
| --- | --- |
| Severity | **LOW** |
| Issue class | Invalid CNAME pointing at bare IP |
| Dangling target | `43.255.154.56` |

The bank's authoritative nameservers still publish this record, but the
service it points to is gone:

```text
$ dig +noall +answer www.devigayatri.bank.in CNAME
www.devigayatri.bank.in. 300  IN  CNAME  43.255.154.56.

$ dig www.devigayatri.bank.in   # -> status: NXDOMAIN
