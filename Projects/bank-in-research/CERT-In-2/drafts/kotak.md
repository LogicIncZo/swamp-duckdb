# Responsible Disclosure: Dangling DNS Records on `KOTAK.bank.in` (5 records)
...
| Entity | Kotak Mahindra Bank Ltd (kotak) |
...
Three hostnames: 811sentry.uat.kotak811 (observability/sentry-pattern name), 811apppurezento-dr.kotak811 (DR environment), www.kapps.kotak, plus nb3dev/nb3uat under kotakuat - all CNAME into *.cdn.cloudflare.net zones that do not resolve.

**Affected hostnames (5 records):** `811sentry.uat.kotak811.bank.in`, `811apppurezento-dr.kotak811.bank.in`, `nb3dev.kotakuat.bank.in`, `nb3uat.kotakuat.bank.in`, `www.kapps.kotak.bank.in`.
...
- Universe: the full published `kotak.bank.in` zone from the IDRBT-derived
...
### F1 - `811sentry.uat.kotak811.bank.in`

| Field | Value |
| --- | --- |
| Severity | **LOW** |
| Issue class | Cloudflare partial-zone dangling configuration (custom-hostname validation required; not claimable) |
| Dangling target | `811sentry.uat.kotak811.bank.in.cdn.cloudflare.net` |

The bank's authoritative nameservers still publish this record, but the
service it points to is gone:

```text
$ dig +noall +answer 811sentry.uat.kotak811.bank.in CNAME
811sentry.uat.kotak811.bank.in.  300 IN  CNAME  811sentry.uat.kotak811.bank.in.cdn.cloudflare.net.

$ dig 811sentry.uat.kotak811.bank.in   # -> status: NOERROR
...
### F2 - `811apppurezento-dr.kotak811.bank.in`

| Field | Value |
| --- | --- |
| Severity | **LOW** |
| Issue class | Cloudflare partial-zone dangling configuration (custom-hostname validation required; not claimable) |
| Dangling target | `811apppurezento-dr.kotak811.bank.in.cdn.cloudflare.net` |

The bank's authoritative nameservers still publish this record, but the
service it points to is gone:

```text
$ dig +noall +answer 811apppurezento-dr.kotak811.bank.in CNAME
811apppurezento-dr.kotak811.bank.in. 1800 IN CNAME 811apppurezento-dr.kotak811.bank.in.cdn.cloudflare.net.

$ dig 811apppurezento-dr.kotak811.bank.in   # -> status: NOERROR
# CNAME target does not resolve (NOERROR); the bank still owns the
# name, so anyone who claims the dangling target serves content here.
```
...
### F3 - `nb3dev.kotakuat.bank.in`

| Field | Value |
| --- | --- |
| Severity | **LOW** |
| Issue class | Cloudflare partial-zone dangling configuration (custom-hostname validation required; not claimable) |
| Dangling target | `nb3dev.kotakuat.bank.in.cdn.cloudflare.net` |

The bank's authoritative nameservers still publish this record, but the
service it points to is gone:

```text
$ dig +noall +answer nb3dev.kotakuat.bank.in CNAME
nb3dev.kotakuat.bank.in. 300  IN  CNAME  nb3dev.kotakuat.bank.in.cdn.cloudflare.net.

$ dig nb3dev.kotakuat.bank.in   # -> status: NOERROR
# CNAME target does not resolve (NOERROR); the bank still owns the
# name, so anyone who claims the dangling target serves content here.
```
...
### F4 - `nb3uat.kotakuat.bank.in`

| Field | Value |
| --- | --- |
| Severity | **LOW** |
| Issue class | Cloudflare partial-zone dangling configuration (custom-hostname validation required; not claimable) |
| Dangling target | `nb3uat.kotakuat.bank.in.cdn.cloudflare.net` |

The bank's authoritative nameservers still publish this record, but the
service it points to is gone:

```text
$ dig +noall +answer nb3uat.kotakuat.bank.in CNAME
nb3uat.kotakuat.bank.in. 300  IN  CNAME  nb3uat.kotakuat.bank.in.cdn.cloudflare.net.

$ dig nb3uat.kotakuat.bank.in   # -> status: NOERROR
# CNAME target does not resolve (NOERROR); the bank still owns the
# name, so anyone who claims the dangling target serves content here.
```
...
### F5 - `www.kapps.kotak.bank.in`

| Field | Value |
| --- | --- |
| Severity | **LOW** |
| Issue class | Cloudflare partial-zone dangling configuration (custom-hostname validation required; not claimable) |
| Dangling target | `www.kapps.kotak.bank.in.cdn.cloudflare.net` |

The bank's authoritative nameservers still publish this record, but the
service it points to is gone:

```text
$ dig +noall +answer www.kapps.kotak.bank.in CNAME
www.kapps.kotak.bank.in. 1800  IN  CNAME  www.kapps.kotak.bank.in.cdn.cloudflare.net.

$ dig www.kapps.kotak.bank.in   # -> status: NOERROR
# CNAME target does not resolve (NOERROR); the bank still owns the
# name, so anyone who claims the dangling target serves content here.
```
...
UAT/DR telemetry disruption; client-side error-report hijack; limited production exposure.
