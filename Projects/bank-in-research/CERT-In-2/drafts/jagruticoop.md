# Responsible Disclosure: Dangling DNS Records on `JAGRUTICOOP.bank.in` (1 records)
...
| Entity | Jagruti Co-operative Bank (jagruticoop) |
...
www.jagruticoop.bank.in CNAMEs directly to 43.255.154.56 - an RFC-violating record many resolvers refuse.

**Affected hostnames (1 records):** `www.jagruticoop.bank.in`.
...
- Universe: the full published `jagruticoop.bank.in` zone from the IDRBT-derived
...
```text
$ dig +noall +answer www.jagruticoop.bank.in CNAME
www.jagruticoop.bank.in. 300  IN  CNAME  43.255.154.56.

$ dig www.jagruticoop.bank.in   # -> status: NXDOMAIN
