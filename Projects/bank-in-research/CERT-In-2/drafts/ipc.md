# Responsible Disclosure: Dangling DNS Records on `IPC.bank.in` (2 records)
...
| Entity | IPC (ipc.bank.in registrant) (ipc) |
...
mail.ipc.bank.in and www.ipc.bank.in both CNAME to the apex, which lacks an address record.

**Affected hostnames (2 records):** `mail.ipc.bank.in`, `www.ipc.bank.in`.
...
- Universe: the full published `ipc.bank.in` zone from the IDRBT-derived
...
| Dangling target | `ipc.bank.in` |

The bank's authoritative nameservers still publish this record, but the
service it points to is gone:

```text
$ dig +noall +answer mail.ipc.bank.in CNAME
mail.ipc.bank.in.  14370  IN  CNAME  ipc.bank.in.

$ dig mail.ipc.bank.in   # -> status: NOERROR
...
```text
$ dig +noall +answer www.ipc.bank.in CNAME
www.ipc.bank.in.  14400  IN  CNAME  ipc.bank.in.

$ dig www.ipc.bank.in   # -> status: NOERROR
# CNAME target does not resolve (NOERROR); the bank still owns the
# name, so anyone who claims the dangling target serves content here.
```
...
Mail/web availability ambiguity.
