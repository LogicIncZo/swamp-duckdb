# Responsible Disclosure: Dangling DNS Records on `CSB.bank.in` (2 records)
...
| Entity | CSB Bank Ltd (formerly Catholic Syrian Bank) (csb) |
...
Both Video-KYC entry points dangle: vkyc.csb.bank.in -> csb-prod-web-alb-1034887934.elb.ap-south-1.amazonaws.com and turnvkyc.csb.bank.in -> csb-prod-video-nlb-* . 'turn-' prefix indicates the Turn.io/AWS video-KYC stack used for RBI-mandated customer verification.

**Affected hostnames (2 records):** `turnvkyc.csb.bank.in`, `vkyc.csb.bank.in`.
...
- Universe: the full published `csb.bank.in` zone from the IDRBT-derived
...
### F1 - `turnvkyc.csb.bank.in`

| Field | Value |
| --- | --- |
| Severity | **CRITICAL** |
| Issue class | AWS ELB/ALB dangling CNAME (reclaimable name) |
| Dangling target | `csb-prod-video-nlb-9f252aff597c1c92.elb.ap-south-1.amazonaws.com` |

The bank's authoritative nameservers still publish this record, but the
service it points to is gone:

```text
$ dig +noall +answer turnvkyc.csb.bank.in CNAME
turnvkyc.csb.bank.in.  1800  IN  CNAME  csb-prod-video-nlb-9f252aff597c1c92.elb.ap-south-1.amazonaws.com.

$ dig turnvkyc.csb.bank.in   # -> status: NXDOMAIN
...
**Why this matters.** The hostname points at an ELB DNS name that no longer resolves. ELB names are drawn from a global pool and can be re-registered by another AWS customer, after which the bank's hostname delivers traffic - including TLS handshakes where certificates can be obtained by the new owner - to attacker-controlled infrastructure.
...
| Severity | **CRITICAL** |
| Issue class | AWS ELB/ALB dangling CNAME (reclaimable name) |
| Dangling target | `csb-prod-web-alb-1034887934.ap-south-1.elb.amazonaws.com` |

The bank's authoritative nameservers still publish this record, but the
service it points to is gone:

```text
$ dig +noall +answer vkyc.csb.bank.in CNAME
vkyc.csb.bank.in.  1800  IN  CNAME  csb-prod-web-alb-1034887934.ap-south-1.elb.amazonaws.com.

$ dig vkyc.csb.bank.in   # -> status: NXDOMAIN
# CNAME target does not resolve (NXDOMAIN); the bank still owns the
# name, so anyone who claims the dangling target serves content here.
```
...
**AWS ELB/ALB dangling CNAME** *(fix within days)*

Remove the dangling record. If the underlying service is still needed, recreate the load balancer and repoint DNS only after end-to-end checks.
...
Video-KYC is the highest-trust interaction a customer has; a hostile claim enables near-perfect KYC phishing and possible capture of applicant documents if clients POST to legacy names.
