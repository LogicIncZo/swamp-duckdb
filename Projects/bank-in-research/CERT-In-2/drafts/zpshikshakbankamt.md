# Responsible Disclosure: Dangling DNS Records on `ZPSHIKSHAKBANKAMT.bank.in` (1 records)
...
| Entity | Zilla Parishad Shikshak Sahakari Bank (AMT) (zpshikshakbankamt) |
...
www.zpshikshakbankamt.bank.in -> zpshikshakbank.bank.in (note: different registrant zone), which does not resolve in the snapshot.

**Affected hostnames (1 records):** `www.zpshikshakbankamt.bank.in`.
...
- Universe: the full published `zpshikshakbankamt.bank.in` zone from the IDRBT-derived
...
### F1 - `www.zpshikshakbankamt.bank.in`

| Field | Value |
| --- | --- |
| Severity | **MEDIUM** |
| Issue class | Dangling CNAME (miscellaneous target) |
| Dangling target | `zpshikshakbank.bank.in` |

The bank's authoritative nameservers still publish this record, but the
service it points to is gone:

```text
$ dig +noall +answer www.zpshikshakbankamt.bank.in CNAME
www.zpshikshakbankamt.bank.in. 3600 IN  CNAME  zpshikshakbank.bank.in.

$ dig www.zpshikshakbankamt.bank.in   # -> status: NXDOMAIN
